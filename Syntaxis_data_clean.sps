...
COMPUTE PTO_OBS=0.
IF ((ANIO='2016' AND MES='12') OR (ANIO='2017' AND (MES='03' OR MES='06'))) PTO_OBS=1.
EXECUTE.
...
respuesta='REFINANCIA' OR respuesta='BANCO')) VAR_DEP=1.
...
IF ((NEW_ACCION=1 OR NEW_ACCION=4) AND VAR_DEP>0) REG_VALIDO=1.
EXECUTE.
...
TEMPORARY.
SELECT IF(PTO_OBS=1 AND REG_VALIDO=1).
CROSSTABS 
  /TABLES= VAR_DEP BY MES
  /FORMAT=AVALUE TABLES 
  /CELLS=COUNT 
